...
| **Factum forum (Superior vs Small Claims)** | `factum-outline.ts` (`FactumForum`, `factumForumFromProcedure`, `SJ_ONLY_ARGUMENT_BLOCK`); forum branches in `factum-section-generator.ts`, `factum-assemble.ts`, and the `sj_factum` whole-doc prompt in `litigation-documents.ts` (`SC_FACTUM_BODY`, `factumForum`) | `factum-selection.ts` drops the SJ-appropriateness node for Small Claims; routes derive forum from `employment.selectedProcedure ?? recommendedProcedure` | same factum routes | `MatterDetailView.tsx` `draftDisplayTitle`/`draftDisplayDescription` relabel the card + workspace to "Small Claims Factum" for a small-claims matter |
| **Editable approved issues** | `employment/issue-catalog.ts` (per-issue labels grouped by gate; `gateName` in `gate-evaluator.ts`) | `routes/employment/intake.ts` (`/issues` POST replaces the approved/dismissed lists) | `/issue-catalog`, `/issues` | `matter/tabs/ApprovedIssuesEditor.tsx` (list approved with remove; add an issue the analysis did not raise) |
| **SOC section-by-section** | `employment/soc-outline.ts` (active nodes rendered + Background Facts; {{para}} strip/ensure marker helpers), `generateSocBackgroundFacts` in `soc-generator.ts` | `soc-generator.ts` `generateNodeAssembledSoc` (uses `req.socDraft` approved/edited overrides; approved Background Facts skips the model call) | `/soc-outline`, `/soc-section/draft` (facts), `/soc-section` (approve/save/clear) | `matter/workspaces/SocOutlinePanel.tsx` (read / edit / Approve per section; draft the Background Facts) |
| **Mediation brief section-by-section** | `employment/mediation-outline.ts` (8 standard sections or firm flow; assemble narrative), `mediation-section-generator.ts` (draft one section) | `litigation-documents.ts` (`mediationNarrativeOverride` skips the model call; deterministic cover/tables/sign-off wrap it), `routes/employment/generators.ts` (assembles from approved sections when all approved) | `/mediation-outline`, `/mediation-section/draft`, `/mediation-section` (approve/save/clear) | `matter/workspaces/MediationOutlinePanel.tsx` (draft / read / edit / Approve / Discard per section) |
