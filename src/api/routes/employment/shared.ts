...
/**
 * What the Statement of Claim reads before it drafts.
 *
 * The Background Facts are the claim's one model-written section, and they are
 * only as particular as the documents they are drafted from. A claim drafted
 * from intake fields alone pleads in generalities, which is what a lawyer
 * means when the result reads mechanically.
 *
 * Shared by the whole-claim route and the section-by-section facts draft so
 * the two cannot disagree about what the claim has read. The section-by-section
 * route passed nothing at all before this existed.
 */
export function buildSocSources(
  matter: Record<string, unknown>,
  briefSourceIds?: string[],
): Array<{ name: string; content: string }> {
  const sources: Array<{ name: string; content: string }> = [];

  // The demand letter on the matter rides along on its own: it is the
  // position already taken, and the claim must not contradict it.
  const dlKey = findGeneratedDocKey(matter, 'demand_letter');
  if (dlKey) {
    const dlHtml = String((matter[dlKey] as Record<string, unknown>)?.html ?? '');
    const dlText = dlHtml.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
    if (dlText.length > 200) sources.push({ name: 'the demand letter on this matter', content: dlText.slice(0, 60_000) });
  }

  // The legacy single attachment, kept working for matters that carry one.
  const attached = matter.socSource as { name?: string; text?: string } | undefined;
  if (attached?.text) sources.push({ name: String(attached.name ?? 'attached document'), content: attached.text.slice(0, 80_000) });

  // The matter's shared store, the same one the mediation brief and
  // Schedule "A" read.
  if (briefSourceIds?.length) {
    const stored = ((matter.briefSources ?? []) as Array<{ id: string; name: string; text: string }>);
    for (const sd of stored) {
      if (!briefSourceIds.includes(sd.id) || !sd.text) continue;
      sources.push({ name: sd.name, content: sd.text.slice(0, 60_000) });
    }
  }
  return sources;
}
