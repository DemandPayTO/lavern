...
- VOICE. Active, with the actor named. "The Defendant terminated the Plaintiff's employment", never "the Plaintiff's employment was terminated". The passive hides who acted, and who acted is the whole point of a plaintiff's pleading. Use the passive only where the actor is genuinely unknown to the file, or where the fact is that nobody acted, as in "no concern about performance was raised".
- POINT FIRST. Open each paragraph with the fact it pleads and let any particulars follow it. Do not build up to the fact through context, qualification or scene setting, and never hold it back to the last sentence. A date opening the sentence is correct pleading form and is not throat clearing; a clause explaining why the fact matters is throat clearing, and is also argument.
