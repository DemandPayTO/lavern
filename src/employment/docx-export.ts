...
function htmlTableToDocx(tableHtml: string, face?: string, cellSize?: number): Table | null {
  ...
  for (const rm of rowMatches) {
    ...
    rows.push(new TableRow({
      children: cellMatches.map(cm => {
        // colspan must survive: the damages table spans value cells across
        // both range columns, and dropping the span shifted every figure
        // one column left in Word.
        const spanMatch = /colspan\s*=\s*"?(\d+)"?/i.exec(cm[2]);
        const isHeader = cm[1].toLowerCase() === 'th';
        // Inline emphasis survives too: [LAWYER: complete] is italic on
        // purpose, and a proofreader must see it that way in Word.
        const runs = parseInlineHtml(cm[3], face, cellSize);
        return new TableCell({
          borders: { top: border, bottom: border, left: border, right: border },
          margins: { top: 60, bottom: 60, left: 100, right: 100 },
          ...(spanMatch ? { columnSpan: Number(spanMatch[1]) } : {}),
          children: [new Paragraph({
            children: runs.length > 0 ? runs : [new TextRun({ text: stripTags(cm[3]), bold: isHeader, font: face, size: cellSize })],
          })],
        });
      }),
    }));
...
export interface HtmlToParagraphsOptions {
   ...
   */
  courtFormat?: boolean;
  /** The document type, where paragraph handling differs by it (the
   *  mediation brief's cover <hr> is a page break). */
  documentType?: string;
  /**
...
export function htmlToParagraphs(
  ...
  for (const block of blocks) {
    ...
    if (/^<hr/i.test(trimmed)) {
      if (options.courtFormat || options.documentType === 'mediation_brief') {
...
export async function htmlToDocx(html: string, options: DocxExportOptions): Promise<Buffer> {
  ...
  const doc = new Document({
    ...
    sections: [{
      // A formal brief's cover page is its own front door: the firm/date
      // header starts on page 2 for the mediation brief.
      ...(options.documentType === 'mediation_brief' ? { properties: { titlePage: true } } : {}),
      headers: {
        ...(options.documentType === 'mediation_brief' ? { first: new Header({ children: [] }) } : {}),
