/**
 * Local Document Assembler — Produces deliverables via on-device Ollama.
 *
 * Parallel to `assembleMistralDocument` and `assembleDocument` (Anthropic).
 * Same retry + structural-validation + LLM-quality-gate logic, but routes
 * every chat call through `localChat` so nothing leaves the box.
 *
 * Cost is always $0 (token counts still tracked for telemetry parity).
 */
...
const logger = createLogger('LOCAL-ASSEMBLY');
...
// ── LLM Quality Gate (Local) ────────────────────────────────────────────

async function localQualityGate(
...
FAIL: [one sentence explaining why]`;
    ...
    const result = await localChat({
      // Local mode doesn't have a separate small/cheap model — same model
      // for orchestration, assembly, and quality gate. Slower than Mistral
      // path, but $0 and on-device.
...
/**
 * Assemble the final deliverable using on-device Ollama.
 *
 * Returns assembled markdown or '' on failure.
 */
export async function assembleLocalDocument(
  session: SessionState,
  request?: LegalRequest,
): Promise<string> {
  ...
  for (let attempt = 1; attempt <= MAX_ASSEMBLY_ATTEMPTS; attempt++) {
    try {
      ...
      // LLM quality gate is skipped on the local provider — the gate asks the
      // model to reply with literally "PASS" or "FAIL: <reason>", which small
      // local models (Gemma, Phi, etc.) reliably fail to follow, blocking
      // every assembly. Structural validation above is still enforced.
      // `localQualityGate` is kept defined for future use if a config flag
      // ever opts back in.
      void localQualityGate;
      logger.info('Quality gate skipped (local provider)', { attempt });
  ...
  session.events.emitEvent({
    type: 'error',
    message: `Document assembly failed after ${MAX_ASSEMBLY_ATTEMPTS} attempts (Local).`,
    source: 'document-assembler',
    timestamp: eventTimestamp(),
  });
